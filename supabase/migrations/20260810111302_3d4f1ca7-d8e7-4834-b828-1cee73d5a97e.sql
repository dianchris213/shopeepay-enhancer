...
GRANT SELECT, INSERT ON public.category_audit_log TO authenticated;
GRANT ALL ON public.category_audit_log TO service_role;
ALTER TABLE public.category_audit_log ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users read own category audit log" ON public.category_audit_log FOR SELECT TO authenticated
  USING (auth.uid() = user_id);
CREATE POLICY "Users append own category audit log" ON public.category_audit_log FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);
CREATE INDEX category_audit_log_user_idx ON public.category_audit_log(user_id, created_at DESC);
