...
describe("category isolation per wallet type", () => {
  ...
  it("never leaks a custom category name into a standard wallet list", () => {
    const names = visibleCategoriesFor({
      categories: all,
      kind: "expense",
      walletType: "Cash",
    }).map((c) => c.name);
  ...
  it("keeps ShopeePay income empty when no category exists yet", () => {
    // No seed data means no fabricated category: the picker stays truly empty.
    const visible = visibleCategoriesFor({
      categories: [],
      kind: "income",
      walletType: "Driver",
