...
import { pushToast, toast } from "@/lib/toast-store";
...
const sortIcons: Record<SortKey, typeof ArrowDownAZ> = {
...
export function CategoryManagerSheet({
  ...
  /** Everything in the current wallet/type scope, before the search filter. */
  const scoped = useMemo(
    () =>
      all
        .filter((c) => c.kind === kind && (c.walletId ?? null) === scope)
        // Mirror the Add Transaction picker: the shared list never shows the
        // custom-wallet-only names, so the two views can never disagree.
        .filter((c) => scope !== null || !isCustomCategory(c.name)),
    [all, kind, scope],
  );
  ...
  const list = useMemo(() => {
    ...
    // Insertion order is the source of truth for "newest first"; the name sorts
    // are locale-aware and stable so the list never jumps between renders.
    const byName = (a: Category, b: Category) =>
      a.name.localeCompare(b.name, undefined, { sensitivity: "base" });
    const next = [...filtered];
    if (sort === "name-asc") next.sort(byName);
    else if (sort === "name-desc") next.sort((a, b) => byName(b, a));
    else next.reverse();
    return next;
  }, [scoped, query, sort]);
    ...
    <Sheet open={open} onClose={handleClose} title="Manage Categories">
      ...
      <div className="mt-5 flex items-baseline justify-between">
        <p className="text-muted-foreground text-[11px] tracking-widest uppercase">
          {kind === "expense" ? t("cm.expenseCats") : t("cm.incomeCats")}
        </p>
        <span className="text-muted-foreground/70 text-[11px] tabular-nums">{list.length}</span>
      </div>
      ...
      {scoped.length > 0 && (
        <div className="mt-2 flex shrink-0 items-center gap-2">
          ...
          <div
            ...
            role="group"
            aria-label={t("cm.sort")}
          >
            {(["name-asc", "name-desc", "newest"] as const).map((key) => {
              ...
              return (
                <button
                  ...
                  aria-label={label}
                  title={label}
                  className={`tap grid size-8 place-items-center rounded-full ${
      ...
      <div className="-mx-1 mt-2 space-y-1.5 px-1 pb-1" aria-live="polite">
        {list.length === 0 && !creating && scoped.length > 0 && (
          <CategoryEmptyState
            variant="no-results"
            testId="cm-no-results"
            title={t("cm.noResults")}
            description={t("cm.noResultsBody")}
            actionLabel={t("cm.reset")}
            onAction={() => setQuery("")}
          />
        )}

        {scoped.length === 0 && !creating && (
          <CategoryEmptyState
            testId="cm-empty"
            title={t("cm.emptyTitle")}
            description={t("cm.emptyBody")}
            actionLabel={t("cm.emptyCta")}
        ...
        {list.map((category) => {
          ...
          return (
            <div
              key={category.id}
              className={`glass rounded-2xl p-3 transition-shadow duration-200 ${
                isEditing ? "ring-primary/40 ring-1" : ""
              }`}
            >
      ...
      <ConfirmDeleteDialog
        ...
        onConfirm={() => {
          if (!pending) return;
          const snapshot = deleteCategory(pending.id);
          setConfirmId(null);
          resetDrafts();
          if (!snapshot) return;
          pushToast({
            ...
            body: snapshot.category.name,
            action: {
              label: t("cm.undo"),
              onClick: () => {
                const result = restoreCategory(snapshot);
                if (result.ok) toast.success(t("toast.categoryRestored"));
                else reportMutation(result, "category", lang, "toast.categoryRestored");
              },
            },
          });
        }}
