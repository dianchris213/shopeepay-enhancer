...
import { checkTransactionCategory } from "@/lib/transaction-guard.functions";
...
export function AddTransactionSheet({ open, onClose }: Props) {
  ...
  /** Assertive screen-reader announcement for rejected submits. */
  ...
  /**
   * Moves keyboard focus into the category picker: the first chip when the
   * list has entries, otherwise the "Settings" quick-create link, otherwise
   * the group itself. From there Tab / Shift+Tab walk the form normally and
   * the arrow keys move between chips.
   */
  function focusCategoryPicker() {
    const group = categoryGroupRef.current;
    if (!group) return;
    const target =
      group.querySelector<HTMLButtonElement>("button:not([data-empty-link])") ??
      group.querySelector<HTMLButtonElement>("button") ??
      group;
    target.focus();
  }

  /** Arrow-key roving between category chips; Tab/Shift+Tab stay native. */
  function onCategoryKeyDown(event: React.KeyboardEvent<HTMLDivElement>) {
    ...
    let next = current < 0 ? 0 : (current + step + chips.length) % chips.length;
  ...
  /** Records a rejected submit for QA: telemetry, counters and the server log. */
  function reportCategoryRejection(reason: string) {
    trackUsage("category_missing_rejected");
    captureTelemetry(
      "add-transaction.category-missing",
      new Error(reason),
      {
        reason,
        kind,
        walletType: selectedWallet?.type ?? null,
        isShopeePay: isShopeePayWallet(selectedWallet),
        hasAmount: amount > 0,
        visibleCategories: visibleCategories.length,
      },
      "warning",
    );
    void checkTransactionCategory({
      data: {
        kind,
        ...
        amount,
        source: "add-transaction-sheet",
      },
    }).catch(() => {
      // Offline or signed-out: the local guard already blocked the submit.
    });
  }
      ...
      if (invalidFields.includes("category")) {
        const reason =
          kind === "expense" && isShopeePayWallet(selectedWallet)
            ...
            : "category_required";
        setAnnouncement(`${t("tx.categoryRequired")}. ${t("tx.categoryRejected")}`);
        pushToast({
          tone: "error",
          title: t("tx.categoryRequired"),
          body: t("tx.categoryRejected"),
        });
        reportCategoryRejection(reason);
        focusCategoryPicker();
      }
      return;
    }
  ...
  return (
    <Sheet open={open} onClose={onClose} title={t("tx.addTitle")}>
      ...
      <div className="mt-5">
        ...
        >
          ...
          {visibleCategories.length === 0 && (
            <div className="col-span-4">
              <CategoryEmptyState
                dense
                testId="tx-empty-categories"
                actionTestId="tx-empty-categories-link"
                title={!selectedWallet ? t("tx.category") : t("cm.emptyTitle")}
                description={
                  !selectedWallet
                    ? t("tx.pickWalletFirst")
                    : `${t("tx.noCategories")} ${t("tx.createCategoryHintPrefix")}`
                }
                {...(selectedWallet
                  ? {
                      actionLabel: t("tx.createCategoryHintLink"),
                      onAction: () => {
                        setQuickCreate(true);
        ...
        {/* Category always starts empty — point the user at the manager. */}
        {visibleCategories.length > 0 && !categoryId && (
          <p
            data-testid="tx-create-category-hint"
            className="text-muted-foreground mt-2 text-[11px] leading-relaxed"
          >
            {t("tx.createCategoryHintPrefix")}{" "}
            <button
              type="button"
            ...
            >
              {t("tx.createCategoryHintLink")}
            </button>
          </p>
        )}

        {/* Assertive announcement for rejected submits (screen readers). */}
        <p
          aria-live="assertive"
          aria-atomic="true"
          role="status"
          data-testid="tx-category-live"
          className="sr-only"
        >
          {announcement}
        </p>
