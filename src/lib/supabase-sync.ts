...
import { recordCategoryAudit } from "@/lib/category-audit";
import {
  getState,
  hydrateState,
  initialState,
...
function syncCategories(prev: Category[], next: Category[]) {
  if (!userId) return;
  // Audit trail first: the before/after snapshot must survive even if the
  // upsert below fails, so the user can still see what was attempted.
  recordCategoryAudit(prev, next, userId);
...
export async function hydrateFromCloud(uid: string) {
  ...
  // New accounts start with an empty category list on purpose; seeding only
  // happens when a build still ships default categories.
  ...
  // Hydration prunes rows seeded by older builds; drop them in the cloud too so
  // they cannot come back on the next sync.
  const kept = new Set(getCategories().map((c) => c.id));
  const stale = categories.filter((c) => !kept.has(c.id)).map((c) => c.id);
  if (stale.length) {
    await supabase.from("categories").delete().in("id", stale).eq("user_id", uid);
  }
